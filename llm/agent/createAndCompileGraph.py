...
from asyncLogsAndMetrics import alogger
from nodesEdgesSchema import Node, Edge, StateSchema, ContextSchema
from callbackHandler import LoggingCallback

async def sessionGraph(thread_id:str, context:ContextSchema):
    try:
        logger, populateMetrics, metric = await alogger(thread_id)
        metric.user.thread_id = thread_id
        node = Node(logger=logger, populateMetrics=populateMetrics)
        edge = Edge()
        callback_manager = CallbackManager(
            handlers=[
                LoggingCallback(logger=logger, populateMetrics=populateMetrics)
                ]
            )
        builder = StateGraph(
